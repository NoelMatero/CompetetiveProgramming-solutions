...
vector<ll> depth;
vector<ll> distance_1;
vector<ll> distance_2;
...
void distance_dfs_1(ll n, ll p) {    
    for (auto c : adj[n]) {
        if (c != p) {            
            depth[c] = depth[n]+1;       
            //cout << c << " " << depth[c] << endl;           
            distance_1[c] = depth[c];     
            distance_dfs_1(c, n);
        }
    }
}
 
void distance_dfs_2(ll n, ll p) {    
    for (auto c : adj[n]) {
        if (c != p) {            
            depth[c] = depth[n]+1;       
            //cout << c << " " << depth[c] << endl;           
            distance_2[c] = depth[c];     
            distance_dfs_2(c, n);
        }
    }
}
...
int main() {
    ...
    depth.clear();
    depth.resize(n);
    distance_dfs_1(furthest_node_1.second, furthest_node_1.second);
    depth.clear();
    depth.resize(n);
    distance_dfs_2(furthest_node_2.second, furthest_node_2.second);
        
 
    for (int i=0;i<n;i++) {
        cout << max(distance_1[i], distance_2[i]) << " ";
    }
}